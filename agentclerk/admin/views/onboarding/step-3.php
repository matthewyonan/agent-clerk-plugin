...
$ac_policies     = $ac_scan_cache['policies'] ?? array();
$ac_gaps         = $ac_scan_cache['gaps'] ?? array();
$ac_support_file = $ac_config['support_file'] ?? '';

if ( empty( $ac_support_file ) && ! empty( $ac_scan_cache ) && class_exists( 'AgentClerk_Scanner' ) ) {
    $ac_support_file = AgentClerk_Scanner::build_support_file( $ac_scan_cache );
}
?>
<div class="wrap ac-wrap">
    ...
    <div class="ac-g2">
            ...
            <div class="ac-card">
                ...
                <div class="ac-card-body" style="padding:14px 17px">
                    <?php if ( ! empty( $ac_products ) ) :
                        $ac_names = array_map( function( $ac_p ) { return esc_html( $ac_p['name'] ) . ' $' . esc_html( number_format( (float) $ac_p['price'], 0 ) ); }, array_slice( $ac_products, 0, 3 ) );
                    ?>
                        <div class="ac-finding"><span class="ac-finding-ok">&#10003;</span><div><strong><?php echo esc_html( count( $ac_products ) . ' products' ); ?></strong> &mdash; <?php echo wp_kses_post( implode( ', ', $ac_names ) ); ?></div></div>
                    <?php endif; ?>
                    <?php if ( ! empty( $ac_policies['refund'] ) ) : ?>
                        <div class="ac-finding"><span class="ac-finding-ok">&#10003;</span><div><strong><?php echo esc_html( 'Refund policy' ); ?></strong> &mdash; <?php echo esc_html( wp_trim_words( $ac_policies['refund'], 10 ) ); ?></div></div>
                    <?php else : ?>
                    ...
                    <?php endif; ?>
                    <?php if ( ! empty( $ac_policies['license'] ) ) : ?>
                        <div class="ac-finding"><span class="ac-finding-ok">&#10003;</span><div><strong><?php echo esc_html( 'License terms' ); ?></strong> &mdash; <?php echo esc_html( wp_trim_words( $ac_policies['license'], 10 ) ); ?></div></div>
                    <?php else : ?>
                    ...
                    <?php endif; ?>
                    <?php if ( ! empty( $ac_policies['delivery'] ) ) : ?>
                        <div class="ac-finding"><span class="ac-finding-ok">&#10003;</span><div><strong><?php echo esc_html( 'Delivery' ); ?></strong> &mdash; <?php echo esc_html( wp_trim_words( $ac_policies['delivery'], 10 ) ); ?></div></div>
                    <?php else : ?>
                    ...
                    <?php endif; ?>
                    <?php foreach ( $ac_gaps as $ac_gap ) : ?>
                        <div class="ac-finding"><span class="ac-finding-warn">&#9888;</span><div><strong><?php echo esc_html( $ac_gap ); ?></strong> &mdash; <?php echo esc_html( 'asking now' ); ?> &rarr;</div></div>
                    <?php endforeach; ?>
                </div>
            </div>
            <div class="ac-card">
                ...
                <div class="ac-card-body">
                    ...
                    <textarea id="ac-support-file" style="min-height:130px;font-size:12px;font-family:'DM Mono',monospace;line-height:1.65"><?php echo esc_textarea( $ac_support_file ); ?></textarea>
        ...
        <div>
            <div class="ac-card" style="display:flex;flex-direction:column;height:560px;max-height:70vh">
                <div class="ac-card-head"><h2><?php echo esc_html( 'Fill the gaps' ); ?></h2><?php if ( ! empty( $ac_gaps ) ) : ?><span class="ac-b ac-b-a"><?php echo esc_html( count( $ac_gaps ) . ' questions' ); ?></span><?php endif; ?></div>
    ...
    <script type="application/json" id="ac-gaps-data"><?php echo wp_json_encode( $ac_gaps ); ?></script>
