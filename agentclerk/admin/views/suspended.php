...
$ac_grace_days   = (int) get_option( 'agentclerk_grace_days_remaining', 0 );
?>
<div class="wrap ac-wrap">
    <div class="ac-card" style="max-width:560px;margin:40px auto;text-align:center;padding:0">
        ...
        <div style="padding:24px">
            <?php if ( $ac_grace_days > 0 ) : ?>
                <div style="margin-bottom:18px;padding:14px;background:var(--amber-lt);border:1px solid #FCD34D;border-radius:var(--r2)">
                    <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:#92400E"><?php printf( esc_html( '%d grace days remaining' ), intval( $ac_grace_days ) ); ?></div>
                    <?php if ( $ac_accrued_fees > 0 ) : ?>
                        <div style="font-size:12px;color:#92400E;margin-top:4px"><?php printf( esc_html( 'Outstanding balance: $%s' ), esc_html( number_format( $ac_accrued_fees, 2 ) ) ); ?></div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
